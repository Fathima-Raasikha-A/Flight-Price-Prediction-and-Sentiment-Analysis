...
- One-hot encoded categorical features (Airline, Source, Destination, Route, Additional_Info); label-encoded the ordinal `Total_Stops` feature
...
- Applied SMOTE **after** the train/test split (fit only on training data) to avoid data leakage — a common pitfall when handling class imbalance
